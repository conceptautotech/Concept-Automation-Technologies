...
function main() {
  console.log("=== DEDUPLICATING TO EXACT INDIAMART PRODUCTS (scraped_data.json) ===");

  const rawData = JSON.parse(fs.readFileSync('scratch/scraped_data.json', 'utf8'));
  const catalogMap = new Map(); // key = modelCode || indiamartId || slug
  ...
  for (const idKey of Object.keys(rawData)) {
    ...
    // Deduplication Key: brand + modelCode (or idKey)
    const dedupKey = `${brand.toLowerCase()}-${modelCode.toLowerCase()}`;
    ...
    if (!catalogMap.has(dedupKey) && !catalogMap.has(slug)) {
      catalogMap.set(dedupKey, {
        id: idKey,
        name: title,
        title: title,
  ...
  console.log(`\n=================================================`);
  console.log(`EXACT UNIQUE INDIAMART PRODUCTS: ${catalogMap.size} PRODUCTS`);
  console.log(`=================================================`);
...
`;
  ...
  console.log(`Successfully written ${formattedProducts.length} exact IndiaMart products to src/data/catalog.ts!`);
}
