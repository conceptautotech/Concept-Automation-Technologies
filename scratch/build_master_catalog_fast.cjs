...
async function main() {
  console.log("=== STARTING COMPLETE MASTER CATALOG HARVEST (INDIAMART + CONCEPTAUTOTECH) ===");
  const catalogMap = new Map();

  // 1. Get Category Links from IndiaMart Mobile
  ...
  console.log(`Harvesting ${categoryList.length} IndiaMart mobile category pages in parallel...`);
      ...
      if (altMatch && altMatch[1]) {
        ...
        if (rawTitle.length > 3 && imgUrl && !catalogMap.has(prodId)) {
          ...
          catalogMap.set(prodId, {
            ...
            description: `${brand} ${rawTitle} industrial component. Genuine high-grade equipment supplied with full warranty and technical support by Concept Automation Technologies.`,
            specs: {
              "Brand": brand,
              "Model": partNumber,
  ...
  console.log(`IndiaMart Scrape Complete -> ${catalogMap.size} unique products harvested!`);

  // 2. Get Sitemap Pages from conceptautotech.com
  console.log("Harvesting conceptautotech.com sitemap...");
  ...
  if (smRes.body) {
    ...
    console.log(`Found ${urls.length} sitemap URLs. Scraping concurrently...`);
    ...
    await mapConcurrent(urls, 25, async (pUrl) => {
      ...
      // Extract all <img src="https://cpimg.tistatic.com/..." alt="..."> or tiimg.tistatic.com
    ...
    console.log(`conceptautotech.com: Extracted ${tistaticCount} additional products with authentic tistatic photos.`);
  }

  console.log(`=== MASTER SCRAPE COMPLETE ===`);
  console.log(`TOTAL PRODUCTS IN MASTER CATALOG: ${catalogMap.size}`);
