...
const CONCURRENCY = 12; // 12 parallel requests
...
function parseProductPage(html, pageUrl) {
  ...
  while ((match = rowRegex.exec(html)) !== null) {
    ...
    if (label && val && label.length < 50 && val.length < 150 && !label.includes('{') && !label.includes('<')) {
  ...
  if (!description || description.length < 20) {
    description = `Original factory sealed ${name}. High-performance industrial automation hardware unit engineered for maximum reliability, panel compatibility, fast response, and zero maintenance downtime. Dispatched directly from Makarba, Ahmedabad warehouse.`;
  }
...
async function run() {
  console.log("=== Fast Parallel Crawler for ConceptAutoTech ===");
  ...
  const seedUrls = [
    ...
    '/proface-hmi.html',
    '/allen-bradley.html',
    '/danfoss-vlt-micro-series-vfd.html',
    '/shaft-encoder.html',
    '/siemens-plc.html',
    '/mitsubishi-hmi.html',
    '/omron-plc.html',
  ...
  console.log("Fetching seed category pages...");
  for (const seed of seedUrls) {
    const res = await fetchUrl(seed);
    if (res.body) {
      const matches = res.body.match(/href="([^"]*-\d+\.html)"/gi) || [];
      matches.forEach(m => {
        const link = m.replace(/href="|"/g, '').trim();
        if (!link.includes('contact') && !link.includes('company')) {
  ...
  console.log(`Discovered ${urlArray.length} distinct real product pages!`);
  ...
  for (let i = 0; i < urlArray.length; i += CONCURRENCY) {
    ...
    const promises = batch.map(async (url) => {
      ...
      if (res.body && res.status === 200) {
        const item = parseProductPage(res.body, url);
        if (item && item.name && item.image) {
          item.id = `im-real-${completed + 1000}`;
    ...
    console.log(`Progress: [${Math.min(completed, urlArray.length)}/${urlArray.length}] products parsed (Scraped: ${scrapedProducts.length})...`);
  }

  console.log(`\nCrawling completed! Total real products scraped: ${scrapedProducts.length}`);

  if (scrapedProducts.length > 0) {
    // Deduplicate scraped products by slug
    const dedupMap = new Map();
    scrapedProducts.forEach(p => {
      if (!dedupMap.has(p.slug)) {
        dedupMap.set(p.slug, p);
      }
    });
    const finalProducts = Array.from(dedupMap.values());
    console.log(`Unique products after deduplication: ${finalProducts.length}`);
    ...
    const updatedCatalog = header + JSON.stringify(finalProducts, null, 2) + ';\n';
    
    fs.writeFileSync(catalogPath + '.backup_real', content);
    fs.writeFileSync(catalogPath, updatedCatalog, 'utf8');
    console.log(`Successfully updated catalog.ts with ${finalProducts.length} 100% REAL scraped products & photos!`);
  }
}
